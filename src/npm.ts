...
export class Npm {
  static async getBundledDependencyLocations(): Promise<Npm.PackageQuery[]> {
    const out = await Cli.execPromise(`npm`, [`query`, `--json`, `":root > .bundled, .bundled .prod"`]);
    Cli.throwIfError(out);
    
    try {
      return JSON.parse(out.stdout);
    } catch (e) {
      console.error(`failed to parse`, out);
      throw e;
    }
  }
}
...
export namespace Npm {
  export type Package = Package.JSONSchemaForNPMPackageJsonFiles;
  
  export interface PackageQuery extends Npm.Package {
    _id: string;
    pkgid: string;
